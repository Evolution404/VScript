...
static void save (LexState *ls, int c) {
  Mbuffer *b = ls->buff;
  if (vsZ_bufflen(b) + 1 > vsZ_sizebuffer(b)) {  // 缓存空间不足了
    size_t newsize;
    if (vsZ_sizebuffer(b) >= MAX_SIZE/2)
      lexerror(ls, "lexical element too long", 0);
    // 将缓存空间扩容为2倍
    newsize = vsZ_sizebuffer(b) * 2;
    vsZ_resizebuffer(ls->L, b, newsize);
  }
  // 将c存进缓存中
  b->buffer[vsZ_bufflen(b)++] = cast(char, c);
}


// 词法分析初始化
// 创建_ENV和所有保留字的字符串对象
void vsX_init (vs_State *L) {
  int i;
  TString *e = vsS_newliteral(L, VS_ENV);  /* create env name */
  vsC_fix(L, obj2gco(e));  /* never collect this name */
  for (i=0; i<NUM_RESERVED; i++) {
    TString *ts = vsS_new(L, vsX_tokens[i]);
    vsC_fix(L, obj2gco(ts));  /* reserved words are never collected */
...
// 从int类型token转换成字符串
const char *vsX_token2str (LexState *ls, int token) {
  if (token < FIRST_RESERVED) {  /* single-byte symbols? 普通字符 */
    vs_assert(token == cast_uchar(token));
    return vsO_pushfstring(ls->L, "'%c'", token);
  }
  else {  // 保留字
    const char *s = vsX_tokens[token - FIRST_RESERVED];  // 从int转换成字符串
    if (token < TK_EOS)  /* fixed format (symbols and reserved words)? eof之前都是固定格式直接压入栈顶 */
      return vsO_pushfstring(ls->L, "'%s'", s);
...
static const char *txtToken (LexState *ls, int token) {
  switch (token) {
    ...
    case TK_FLT: case TK_INT:
      save(ls, '\0');
      return vsO_pushfstring(ls->L, "'%s'", vsZ_buffer(ls->buff));
    default:  // 这个符号的字符串都是固定的
      return vsX_token2str(ls, token);
  }
}


static l_noret lexerror (LexState *ls, const char *msg, int token) {
  msg = vsG_addinfo(ls->L, msg, ls->source, ls->linenumber);
  if (token)
    vsO_pushfstring(ls->L, "%s near %s", msg, txtToken(ls, token));
  vsD_throw(ls->L, VS_ERRSYNTAX);
}


l_noret vsX_syntaxerror (LexState *ls, const char *msg) {
...
TString *vsX_newstring (LexState *ls, const char *str, size_t l) {
  vs_State *L = ls->L;
  TValue *o;  /* entry for 'str' */
  TString *ts = vsS_newlstr(L, str, l);  /* create new string */
  setsvalue2s(L, L->top++, ts);  /* temporarily anchor it in stack */
  o = vsH_set(L, ls->h, L->top - 1);  // 查询ls->h以str为键对应的value
...
static void inclinenumber (LexState *ls) {
  int old = ls->current;
  vs_assert(currIsNewline(ls));  // 确保ls->current是换行符
  next(ls);  /* skip '\n' or '\r' 跳过换行符 */
...
void vsX_setinput (vs_State *L, LexState *ls, ZIO *z, TString *source,
                    int firstchar) {
  ls->t.token = 0;
  ls->L = L;
  ...
  ls->lastline = 1;
  ls->source = source;
  ls->envn = vsS_newliteral(L, VS_ENV);  /* get env name */
  vsZ_resizebuffer(ls->L, ls->buff, VS_MINBUFFER);  /* initialize buffer */
}
...
static int check_next2 (LexState *ls, const char *set) {
  vs_assert(set[2] == '\0');  // set字符串一定只有两个字符
...
/* VS_NUMBER */
/*
** this function is quite liberal in what it accepts, as 'vsO_str2num'
...
static int read_numeral (LexState *ls, SemInfo *seminfo) {
  ...
  save(ls, '\0');
  if (vsO_str2num(vsZ_buffer(ls->buff), &obj) == 0)  /* format error? */
...
static void read_long_string (LexState *ls, SemInfo *seminfo, int sep) {
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '\n': case '\r': {
        save(ls, '\n');
        inclinenumber(ls);
        if (!seminfo) vsZ_resetbuffer(ls->buff);  /* avoid wasting space 注释没必要浪费空间保存 */
        break;
      }
      default: {  // 在识别实际的内容
  ...
  if (seminfo)
    ...
    seminfo->ts = vsX_newstring(ls, vsZ_buffer(ls->buff) + (2 + sep),
                                     vsZ_bufflen(ls->buff) - 2*(2 + sep));
}
...
static int gethexa (LexState *ls) {
  ...
  return vsO_hexavalue(ls->current);
}
...
static int readhexaesc (LexState *ls) {
  ...
  vsZ_buffremove(ls->buff, 2);  /* remove saved chars from buffer */
  return r;
}


// 读取一个utf8值 返回该utf8的10进制值
// 格式为\u{ffff}  ffff代表n个16进制数 如vs交互窗口输入 '\u{607c}' 打印 恼
// 执行结束current指向}的后一个
static unsigned long readutf8esc (LexState *ls) {
  ...
  while ((save_and_next(ls), lisxdigit(ls->current))) {
    i++;
    r = (r << 4) + vsO_hexavalue(ls->current);  // 计算16进制值
    esccheck(ls, r <= 0x10FFFF, "UTF-8 value too large");
  }
  // 此时缓存 \u{ffff  ffff代表n个16进制数
  esccheck(ls, ls->current == '}', "missing '}'");
  next(ls);  /* skip '}' 没有保存另一半大括号} */
  vsZ_buffremove(ls->buff, i);  /* remove saved chars from buffer 清空之前保存的值 */
  return r;
}
...
static void utf8esc (LexState *ls) {
  ...
  int n = vsO_utf8esc(buff, readutf8esc(ls));
...
static int readdecesc (LexState *ls) {
  ...
  vsZ_buffremove(ls->buff, i);  /* remove read digits from buffer */
  return r;
}
...
static void read_string (LexState *ls, int del, SemInfo *seminfo) {
      ...
      case '\\': {  /* escape sequences 读到反斜杠进行特殊处理 */
        ...
        switch (ls->current) {
          ...
          case 'z': {  /* zap following span of spaces \z可以省略掉它后面的空白符 */
            vsZ_buffremove(ls->buff, 1);  /* remove '\\' */
            next(ls);  /* skip the 'z' */
       ...
       only_save:  // 不读取下一个字符只保存当前字符c
         vsZ_buffremove(ls->buff, 1);  /* remove '\\' 删掉预先保存的反斜杠 */
         save(ls, c);  // 保存当前字符c
         /* go through */
       no_save: break;  // 什么都不做
      }
  ...
  seminfo->ts = vsX_newstring(ls, vsZ_buffer(ls->buff) + 1,
                                   vsZ_bufflen(ls->buff) - 2);
}
    ...
    switch (ls->current) {  // 根据当前的字符进行不同处理
      ...
      case '-': {  /* '-' or '--' (comment) */
        ...
        if (ls->current == '[') {  /* long comment? 此时在多行注释的第一个[ */
          ...
          int sep = skip_sep(ls);
          // 此时在多行注释的第二个[
          vsZ_resetbuffer(ls->buff);  /* 'skip_sep' may dirty the buffer 注释不需要保存内容 */
          if (sep >= 0) {
            read_long_string(ls, NULL, sep);  /* skip long comment 跳过整个注释 current指向第二个]的下一个 */
            vsZ_resetbuffer(ls->buff);  /* previous call may dirty the buff. */
            break;
          }
        }
      ...
      default: {
        if (lislalpha(ls->current)) {  /* identifier or reserved word? */
          ...
          ts = vsX_newstring(ls, vsZ_buffer(ls->buff),
                                  vsZ_bufflen(ls->buff));
          seminfo->ts = ts;
...
// 调用时ls必须没有向前看信息
int vsX_lookahead (LexState *ls) {
  vs_assert(ls->lookahead.token == TK_EOS);  // 确保没有向前看的信息
