...
// 在vsX_newstring函数中确保了字符串一样地址就一样
// 该宏传入的TString对象一定由vsX_newstring创建而成,所以可以直接比较地址就能判断是否相等
...
static l_noret semerror (LexState *ls, const char *msg) {
  ...
  vsX_syntaxerror(ls, msg);
}


static l_noret error_expected (LexState *ls, int token) {
  vsX_syntaxerror(ls,
      vsO_pushfstring(ls->L, "%s expected", vsX_token2str(ls, token)));
}
...
static l_noret errorlimit (FuncState *fs, int limit, const char *what) {
  ...
  const char *where = (line == 0)
                      ? "main function"
                      : vsO_pushfstring(L, "function at line %d", line);
  msg = vsO_pushfstring(L, "too many %s (limit is %d) in %s",
                             what, limit, where);
  vsX_syntaxerror(fs->ls, msg);
}
...
// c为真报错 c为假无操作
#define check_condition(ls,c,msg)	{ if (!(c)) vsX_syntaxerror(ls, msg); }
...
static void check_match (LexState *ls, int what, int who, int where) {
  if (!testnext(ls, what)) {
    if (where == ls->linenumber)
      ...
      vsX_syntaxerror(ls, vsO_pushfstring(ls->L,
             "%s expected (to close %s at line %d)",
              vsX_token2str(ls, what), vsX_token2str(ls, who), where));
    }
  }
}
...
static void codestring (LexState *ls, expdesc *e, TString *s) {
  init_exp(e, VK, vsK_stringK(ls->fs, s));
}
...
static void new_localvar (LexState *ls, TString *name) {
  ...
  vsM_growvector(ls->L, dyd->actvar.arr, dyd->actvar.n + 1,
...
static void new_localvarliteral_ (LexState *ls, const char *name, size_t sz) {
  new_localvar(ls, vsX_newstring(ls, name, sz));
}
...
static int newupvalue (FuncState *fs, TString *name, expdesc *v) {
  ...
  // 如果有必要则申请新的空间
  vsM_growvector(fs->ls->L, f->upvalues, fs->nups, f->sizeupvalues,
...
static void singlevar (LexState *ls, expdesc *var) {
  ...
  if (var->k == VVOID) {  /* global name? 全局变量 */
    ...
    vs_assert(var->k != VVOID);  /* this one must exist */
    // 初始化key为VK类型 将varname放入常量表 key->u.info存放varname在常量表的位置
    codestring(ls, &key, varname);  /* key is variable name */
    // 设置var为VINDEXED
    vsK_indexed(fs, var, &key);  /* env[varname] */
  }
}
...
static void adjust_assign (LexState *ls, int nvars, int nexps, expdesc *e) {
  ...
  if (hasmultret(e->k)) {
    ...
    if (extra < 0) extra = 0;
    // 指定表达式的返回值个数为extra
    vsK_setreturns(fs, e, extra);  /* last exp. provides the difference */
    ...
    // VVARARG类型在上一行vsK_setmultret额外分配了一个寄存器
    // 所以最终减少了一个寄存器
    if (extra > 1) vsK_reserveregs(fs, extra-1);
  }
  else {
    // 处理掉表达式e 将表达式的值存入寄存器
    if (e->k != VVOID) vsK_exp2nextreg(fs, e);  /* close last expression */
    if (extra > 0) {  // 如果变量比表达式多 多的变量赋值为nil
      int reg = fs->freereg;
      vsK_reserveregs(fs, extra);
      vsK_nil(fs, reg, extra);  // 从reg开始extra个寄存器设置为nil
    }
  }
...
static void enterlevel (LexState *ls) {
  vs_State *L = ls->L;
  ++L->nCcalls;
  checklimit(ls->fs, L->nCcalls, VSI_MAXCCALLS, "C levels");
}
...
static void closegoto (LexState *ls, int g, Labeldesc *label) {
  ...
  vs_assert(eqstr(gt->name, label->name));  // 确保传入的label和goto是匹配的
  // goto位置的变量个数不能小于label位置的变量个数
  // 也就是goto不能向后跳且中间有变量定义
  if (gt->nactvar < label->nactvar) {
    TString *vname = getlocvar(fs, gt->nactvar)->varname;
    const char *msg = vsO_pushfstring(ls->L,
      "<goto %s> at line %d jumps into the scope of local '%s'",
      getstr(gt->name), gt->line, getstr(vname));
    semerror(ls, msg);
  }
  // 跳转目标是label语句所在的指令
  vsK_patchlist(fs, gt->pc, label->pc);
  ...
  for (i = bl->firstlabel; i < dyd->label.n; i++) {  // 遍历当前块内的label
    ...
    if (eqstr(lb->name, gt->name)) {  /* correct label? lable和goto的名称相等 */
      ...
      if (gt->nactvar > lb->nactvar &&
        ...
        vsK_patchclose(ls->fs, gt->pc, lb->nactvar);
...
static int newlabelentry (LexState *ls, Labellist *l, TString *name,
  ...
  int n = l->n;
  // 如果满了就增长空间
  vsM_growvector(ls->L, l->arr, n, l->size,
  ...
  while (i < gl->n) {
    ...
    if (gt->nactvar > bl->nactvar) {
      if (bl->upval)
        ...
        vsK_patchclose(fs, gt->pc, bl->nactvar);
...
static void enterblock (FuncState *fs, BlockCnt *bl, lu_byte isloop) {
  ...
  bl->previous = fs->bl;
  fs->bl = bl;
  vs_assert(fs->freereg == fs->nactvar);
}
...
static l_noret undefgoto (LexState *ls, Labeldesc *gt) {
  ...
  msg = vsO_pushfstring(ls->L, msg, getstr(gt->name), gt->line);
  semerror(ls, msg);
}
...
static void leaveblock (FuncState *fs) {
  ...
  if (bl->previous && bl->upval) {
    ...
    vsK_patchclose(fs, j, bl->nactvar);
    vsK_patchtohere(fs, j);
  }
  // break语句只有在循环块中才有效
  ...
  vs_assert(bl->nactvar == fs->nactvar);
  // 释放掉这个块用到的临时寄存器
...
static Proto *addprototype (LexState *ls) {
  ...
  if (fs->np >= f->sizep) {  // 数组已经放满了
    // 增加Proto数组的空间
    int oldsize = f->sizep;
    vsM_growvector(L, f->p, fs->np, f->sizep, Proto *, MAXARG_Bx, "functions");
    while (oldsize < f->sizep)
      f->p[oldsize++] = NULL;
  }
  // 往子函数(p)数组中增加新建的Proto对象
  f->p[fs->np++] = clp = vsF_newproto(L);
  vsC_objbarrier(L, f, clp);
  return clp;
}
...
static void codeclosure (LexState *ls, expdesc *v) {
  ...
  init_exp(v, VRELOCABLE, vsK_codeABx(fs, OP_CLOSURE, 0, fs->np - 1));
  // 回填OP_CLOSURE指令,参数A是freereg,然后freereg++
  vsK_exp2nextreg(fs, v);  /* fix it at the last register */
}
...
static void close_func (LexState *ls) {
  ...
  // OP_RETURN指令的参数
  vsK_ret(fs, 0, 0);  /* final return */
  leaveblock(fs);
  // 避免浪费空间 将各个数组的大小都重新分配成它当前存的元素个数
  // 重新分配指令数组
  vsM_reallocvector(L, f->code, f->sizecode, fs->pc, Instruction);
  f->sizecode = fs->pc;
  // 重新分配行信息数组
  vsM_reallocvector(L, f->lineinfo, f->sizelineinfo, fs->pc, int);
  f->sizelineinfo = fs->pc;
  // 重新分配常量数组
  vsM_reallocvector(L, f->k, f->sizek, fs->nk, TValue);
  f->sizek = fs->nk;
  // 重新分配子函数数组
  vsM_reallocvector(L, f->p, f->sizep, fs->np, Proto *);
  f->sizep = fs->np;
  // 重新分配局部变量数组
  vsM_reallocvector(L, f->locvars, f->sizelocvars, fs->nlocvars, LocVar);
  f->sizelocvars = fs->nlocvars;
  // 重新分配upvalue数组
  vsM_reallocvector(L, f->upvalues, f->sizeupvalues, fs->nups, Upvaldesc);
  f->sizeupvalues = fs->nups;
  // 执行该函数时应该只有块栈应该只有一层,调用过leaveblock后fs->bl应为NULL
  vs_assert(fs->bl == NULL);
  // 退出当前函数
  ls->fs = fs->prev;
  vsC_checkGC(L);
}
...
static void fieldsel (LexState *ls, expdesc *v) {
  ...
  FuncState *fs = ls->fs;
  expdesc key;
  vsK_exp2anyregup(fs, v);  // 把原来的表达式存到寄存器
  vsX_next(ls);  /* skip the dot or colon 跳过点或冒号 */
  checkname(ls, &key);  // 读取点或冒号后的变量名,设置key为VK类型
  // 设置v为v[key]
  // 设置v的类型为VINDEXED
  vsK_indexed(fs, v, &key);
}
...
static void recfield (LexState *ls, struct ConsControl *cc) {
  ...
  cc->nh++;  // 哈希部分的个数应该加1
  checknext(ls, '=');
  rkkey = vsK_exp2RK(fs, &key);  // 得到key在寄存器或常量表中的位置
  expr(ls, &val);  // 读取var
  // 生成OP_SETTABLE指令
  //                              表的位置     key的位置    val的位置
  vsK_codeABC(fs, OP_SETTABLE, cc->t->u.info, rkkey, vsK_exp2RK(fs, &val));
...
static void closelistfield (FuncState *fs, struct ConsControl *cc) {
  ...
  // 把cc->v存储的变量放到寄存器里
  vsK_exp2nextreg(fs, &cc->v);
  cc->v.k = VVOID;
  if (cc->tostore == LFIELDS_PER_FLUSH) {  // 到了50个该刷新了 
    vsK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);  /* flush */
...
static void lastlistfield (FuncState *fs, struct ConsControl *cc) {
  ...
  if (hasmultret(cc->v.k)) {
    vsK_setmultret(fs, &cc->v);  // 回填指令
    vsK_setlist(fs, cc->t->u.info, cc->na, VS_MULTRET);  // OP_SETLIST指令设置为参数个数不定也就是参数C是0
  ...
  // 正常情况
    ...
    if (cc->v.k != VVOID)  // 最后一项就是普通的表达式
      vsK_exp2nextreg(fs, &cc->v);  // 存到寄存器中
    vsK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);  // 生成并设置OP_SETLIST指令的参数
  }
}
...
static void field (LexState *ls, struct ConsControl *cc) {
  ...
  switch(ls->t.token) {
    ...
    case TK_NAME: {  /* may be 'listfield' or 'recfield' */
      // 根据有没有等号决定类型
      if (vsX_lookahead(ls) != '=')  /* expression? 识别TK_NAME后面是不是= */
...
static void constructor (LexState *ls, expdesc *t) {
  ...
  int pc = vsK_codeABC(fs, OP_NEWTABLE, 0, 0, 0);
  ...
  // 将生成的表保存到寄存器中
  vsK_exp2nextreg(ls->fs, t);  /* fix it at stack top */
  checknext(ls, '{');  // 跳过{开始识别 内部结构
  // 循环识别表中的每个域(field)
  do {
    // 除非第一次进来的时候cc.v是VVOID类型,tostore必须大于0
    vs_assert(cc.v.k == VVOID || cc.tostore > 0);
  ...
  SETARG_B(fs->f->code[pc], vsO_int2fb(cc.na)); /* set initial array size */
  SETARG_C(fs->f->code[pc], vsO_int2fb(cc.nh));  /* set initial table size */
}
...
static void parlist (LexState *ls) {
  ...
  if (ls->t.token != ')') {  /* is 'parlist' not empty? */
    ...
    do {
      switch (ls->t.token) {
        ...
        default: vsX_syntaxerror(ls, "<name> or '...' expected");
      }
  ...
  // 准备出来函数参数的空间
  vsK_reserveregs(fs, fs->nactvar);  /* reserve register for parameters */
}
...
static void body (LexState *ls, expdesc *e, int ismethod, int line) {
  ...
  // 新建Proto对象
  // 这里不能直接调用vsF_newproto因为需要在外部函数(ls->fs)的子函数数组中加入新建项
...
static void funcargs (LexState *ls, expdesc *f, int line) {
  ...
  switch (ls->t.token) {
    ...
    case TK_STRING: {  /* funcargs -> STRING */
      ...
      vsX_next(ls);  /* must use 'seminfo' before 'next' */
      break;
    }
    default: {
      vsX_syntaxerror(ls, "function arguments expected");
    }
  }
  // 在switch语句之后还剩下最后一个参数需要保存
  vs_assert(f->k == VNONRELOC);
  // f->u.info现在保存被调用的函数 所以base就是OP_CALL指令的参数A
  base = f->u.info;  /* base register for call */
  // 计算函数的参数个数
  if (hasmultret(args.k))  // 最后一个参数是函数调用或者...说明函数的参数个数不定
    nparams = VS_MULTRET;  /* open call */
  else {  // 有确定的参数个数
    if (args.k != VVOID)  // VVOID类型不用保存到寄存器上
      vsK_exp2nextreg(fs, &args);  /* close last argument */
  ...
  // 参数B:含义是参数个数+1
  // 参数C:默认有一个返回值
  init_exp(f, VCALL, vsK_codeABC(fs, OP_CALL, base, nparams+1, 2));
  vsK_fixline(fs, line);
  // 函数调用完成就剩下一个返回值
...
static void primaryexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    // 既不是变量名也不是左括号报错
    default: {
      vsX_syntaxerror(ls, "unexpected symbol");
    }
  }
}
...
static void simpleexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    case TK_DOTS: {  /* vararg ... 识别到可变参数 */
      ...
      init_exp(v, VVARARG, vsK_codeABC(fs, OP_VARARG, 0, 1, 0));
      break;
    }
    // 剩下三种情况使用return是因为他们都已经读取了下一个符号 不需要执行vsX_next
    case '{': {  /* constructor 识别表初始化结构 */
      constructor(ls, v);
      return;
    }
    case TK_FUNCTION: {  // 识别函数
      vsX_next(ls);  // 跳过function关键字
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  while (op != OPR_NOBINOPR && priority[op].left > limit) {
    ...
    int line = ls->linenumber;
    vsX_next(ls);
    vsK_infix(ls->fs, op, v);  // 根据操作符设置第一个操作数
...
static void check_conflict (LexState *ls, struct LHS_assign *lh, expdesc *v) {
  ...
  if (conflict) {
    ...
    vsK_codeABC(fs, op, extra, v->u.info, 0);
...
static void assignment (LexState *ls, struct LHS_assign *lh, int nvars) {
  ...
  if (testnext(ls, ',')) {  /* assignment -> ',' suffixedexp assignment */
    ...
    checklimit(ls->fs, nvars + ls->L->nCcalls, VSI_MAXCCALLS,
                    "C levels");
    // 递归识别suffixedexp
  ...
  // 开始识别等号后面
    ...
    if (nexps != nvars)
      ...
      vsK_setoneret(ls->fs, &e);  /* close last expression */
      vsK_storevar(ls->fs, &lh->v, &e);
...
static int cond (LexState *ls) {
  ...
  vsK_goiftrue(ls->fs, &v);
  return v.f;
}


// 识别goto语句和break语句
// vs的break语句本质上就是一个goto语句
...
static void gotostat (LexState *ls, int pc) {
  ...
  if (testnext(ls, TK_GOTO))
    ...
    vsX_next(ls);  /* skip break */
    // 为break语句创建break字符串
    label = vsS_new(ls->L, "break");
  }
  // 向goto列表中加入新的项
...
static void checkrepeated (FuncState *fs, Labellist *ll, TString *label) {
  ...
  for (i = fs->bl->firstlabel; i < ll->n; i++) {
    if (eqstr(label, ll->arr[i].name)) {
      const char *msg = vsO_pushfstring(fs->ls->L,
...
static void labelstat (LexState *ls, TString *label, int line) {
  ...
  l = newlabelentry(ls, ll, label, line, vsK_getlabel(fs));
...
static void whilestat (LexState *ls, int line) {
  ...
  BlockCnt bl;
  // 跳过while
  vsX_next(ls);  /* skip WHILE */
  // whileinit就是当前的pc,也就是while的条件表达式的第一条指令
  whileinit = vsK_getlabel(fs);
  ...
  vsK_patchtohere(fs, condexit);  /* false conditions finish the loop */
}
...
static void repeatstat (LexState *ls, int line) {
  ...
  int condexit;
  FuncState *fs = ls->fs;
  int repeat_init = vsK_getlabel(fs);
  BlockCnt bl1, bl2;
  enterblock(fs, &bl1, 1);  /* loop block */
  enterblock(fs, &bl2, 0);  /* scope block */
  vsX_next(ls);  /* skip REPEAT */
  statlist(ls);
  ...
  if (bl2.upval)  /* upvalues? */
    // 回填JMP指令的参数A
    vsK_patchclose(fs, condexit, bl2.nactvar);
  leaveblock(fs);  /* finish scope */
  // 条件表达式为假的时候跳转到repeat_init位置
  vsK_patchlist(fs, condexit, repeat_init);  /* close the loop */
...
static int exp1 (LexState *ls) {
  ...
  int reg;
  expr(ls, &e);
  vsK_exp2nextreg(ls->fs, &e);
  vs_assert(e.k == VNONRELOC);
  reg = e.u.info;
  return reg;
}
...
static void forbody (LexState *ls, int base, int line, int nvars, int isnum) {
  ...
  prep = isnum ? vsK_codeAsBx(fs, OP_FORPREP, base, NO_JUMP) : vsK_jump(fs);
  enterblock(fs, &bl, 0);  /* scope for declared variables */
  // 根据传入的nvars调整额外定义的变量,额外变量的作用域在被包裹的区域内
  adjustlocalvars(ls, nvars);
  vsK_reserveregs(fs, nvars);
  block(ls);
  leaveblock(fs);  /* end of scope for declared variables */
  // for循环包裹块的前部指令OP_FORPREP或者OP_JMP都需要跳转到后部
  vsK_patchtohere(fs, prep);
  // 第一种由OP_FORLOOP结束
  if (isnum)  /* numeric for? */
    endfor = vsK_codeAsBx(fs, OP_FORLOOP, base, NO_JUMP);
  // 第二种由OP_TFORCALL和OP_TFORLOOP结束
  else {  /* generic for */
    vsK_codeABC(fs, OP_TFORCALL, base, 0, nvars);
    // 设置OP_TAILCALL的行位置
    vsK_fixline(fs, line);
    endfor = vsK_codeAsBx(fs, OP_TFORLOOP, base + 2, NO_JUMP);
  }
  // 两种类型后部指令都需要跳转到前部指令的下一条指令
  vsK_patchlist(fs, endfor, prep + 1);
  // 生成尾部指令的行是for循环的开始位置
  vsK_fixline(fs, line);
}
...
static void fornum (LexState *ls, TString *varname, int line) {
  ...
  if (testnext(ls, ','))  // 有步长直接解析
    ...
    // 将常量1放入寄存器
    vsK_codek(fs, fs->freereg, vsK_intK(fs, 1));
    vsK_reserveregs(fs, 1);
  }
  // 这种类型的for循环定义了一个额外变量
  // var=exp1定义的变量var
...
static void forlist (LexState *ls, TString *indexname) {
  ...
  vsK_checkstack(fs, 3);  /* extra space to call generator */
...
static void test_then_block (LexState *ls, int *escapelist, int line) {
  ...
  // 处理正常的if,进去不直接是goto或者break
    ...
    vsK_goiftrue(ls->fs, &v);  /* skip over block if condition is false */
    enterblock(fs, &bl, 0);
  ...
  if (ls->t.token == TK_ELSE ||
    ...
    vsK_concat(fs, escapelist, vsK_jump(fs));  /* must jump over it */
  // 已经解析完这个块的语句了,下一条语句就是falselist中要回填的位置
  vsK_patchtohere(fs, jf);
}
...
static void funcstat (LexState *ls, int line) {
  ...
  int ismethod;
  expdesc v, b;
  vsX_next(ls);  /* skip FUNCTION */
  // 表达式v代表要被赋值的变量
  ismethod = funcname(ls, &v);
  // 表达式b用来保存生成的函数的位置
  body(ls, &b, ismethod, line);
  // 将函数进行赋值
  vsK_storevar(ls->fs, &v, &b);
  vsK_fixline(ls->fs, line);  /* definition "happens" in the first line */
}
...
static void retstat (LexState *ls) {
  ...
  if (block_follow(ls, 1) || ls->t.token == ';')
    ...
    // nret现在是表达式个数,如果是多返回值表达式nret需要设置为VS_MULTRET
    ...
    if (hasmultret(e.k)) {
      ...
      if (e.k == VCALL && nret == 1) {  /* tail call? */
        ...
        vs_assert(GETARG_A(getinstruction(fs,&e)) == fs->nactvar);
      }
      first = fs->nactvar;
      nret = VS_MULTRET;  /* return all values */
    }
    else {
      // 这里进行了优化
      ...
      if (nret == 1)  /* only one single value? */
        first = vsK_exp2anyreg(fs, &e);
      else {
        vsK_exp2nextreg(fs, &e);  /* values must go to the stack */
        first = fs->nactvar;  /* return all active values */
        vs_assert(nret == fs->freereg - first);
      }
    }
  }
  // 生成最后的OP_RETURN指令
  vsK_ret(fs, first, nret);
  // 跳过return语句最后可能的分号
...
static void statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_LOCAL: {  /* stat -> localstat 识别定义局部变量并赋值或者定义局部函数 */
      vsX_next(ls);  /* skip LOCAL 识别下一个符号 丢弃识别的local */
    ...
    case TK_DBCOLON: {  /* stat -> label 识别到双冒号是标签声明语句 */
      vsX_next(ls);  /* skip double colon */
      labelstat(ls, str_checkname(ls), line);
      break;
    }
    // 识别return语句
    case TK_RETURN: {  /* stat -> retstat */
      vsX_next(ls);  /* skip RETURN */
      retstat(ls);
      break;
    }
    case TK_BREAK:   /* stat -> breakstat */
    case TK_GOTO: {  /* stat -> 'goto' NAME */
      gotostat(ls, vsK_jump(ls->fs));
      break;
    }
  ...
  vs_assert(ls->fs->f->maxstacksize >= ls->fs->freereg &&
...
static void mainfunc (LexState *ls, FuncState *fs) {
  BlockCnt bl;
  expdesc v;
  // vsY_parser中新建了FuncState和Propo对象,这里只新定义了BlockCnt就可以调用open_func
  open_func(ls, fs, &bl);
...
LClosure *vsY_parser (vs_State *L, ZIO *z, Mbuffer *buff,
  ...
  LClosure *cl = vsF_newLclosure(L, 1);  /* create main closure 主函数闭包 */
  // 将主函数闭包压入栈顶 栈顶自增, 放入栈顶等待之后的调用
  setclLvalue(L, L->top, cl);  /* anchor it (to avoid being collected) */
  vsD_inctop(L);
  // 创建空表 用于缓存常量值
  lexstate.h = vsH_new(L);  /* create table for scanner */
  // 将该空表压入栈顶 栈顶自增
  sethvalue(L, L->top, lexstate.h);  /* anchor it */
  vsD_inctop(L);
  // 初始化Propo对象
  // 只有主函数的Proto对象通过vsF_newproto函数直接创建
  // 其他函数均在body函数中调用addprototype函数创建
  // 因为主函数不是任何函数的子函数,不用加入到f->p数组内
  funcstate.f = cl->p = vsF_newproto(L);
  // source保存文件名(@xxx) xxx是文件名
  funcstate.f->source = vsS_new(L, name);  /* create and anchor TString */
  vs_assert(iswhite(funcstate.f));  /* do not need barrier here */
  lexstate.buff = buff;
  lexstate.dyd = dyd;
  // 初始化dyd中三个列表的大小为0
  dyd->actvar.n = dyd->gt.n = dyd->label.n = 0;
  // 为词法分析设置输入文件
  vsX_setinput(L, &lexstate, z, funcstate.f->source, firstchar);
  ...
  vs_assert(!funcstate.prev && funcstate.nups == 1 && !lexstate.fs);
  /* all scopes should be correctly finished */
  vs_assert(dyd->actvar.n == 0 && dyd->gt.n == 0 && dyd->label.n == 0);
  // 去掉词法分析器使用的表
