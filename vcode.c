...
/* Maximum number of registers in a VS function (must fit in 8 bits) */
// 一个函数寄存器个数的最大值
#define MAXREGS		255
...
void vsK_nil (FuncState *fs, int from, int n) {
  ...
  vsK_codeABC(fs, OP_LOADNIL, from, n - 1, 0);  /* else no optimization */
}
...
** (Jump addresses are relative in VS)
*/
// 设置跳转指令pc跳转到dest指令
...
static void fixjump (FuncState *fs, int pc, int dest) {
  ...
  vs_assert(dest != NO_JUMP);
  if (abs(offset) > MAXARG_sBx)
    vsX_syntaxerror(fs->ls, "control structure too long");
...
void vsK_concat (FuncState *fs, int *l1, int l2) {
...
int vsK_jump (FuncState *fs) {
  ...
  // 生成跳转指令
  j = vsK_codeAsBx(fs, OP_JMP, 0, NO_JUMP);
  // 将新生成的跳转指令插到原来的回填链头
  vsK_concat(fs, &j, jpc);  /* keep them on hold */
  // 返回新生成的跳转指令或者说一条回填链
  return j;
}
...
*/
// 生成OP_RETURN指令
void vsK_ret (FuncState *fs, int first, int nret) {
  // 参数B是返回值个数+1
  vsK_codeABC(fs, OP_RETURN, first, nret+1, 0);
}
...
// 修改lasttarget为pc
void vsK_patchtohere (FuncState *fs, int list) {
  // 标记pc为上一次跳转目标
  vsK_getlabel(fs);  /* mark "here" as a jump target */
  vsK_concat(fs, &fs->jpc, list);
}
...
// 否则使用target回填list回填链
void vsK_patchlist (FuncState *fs, int list, int target) {
  // 如果目标位置就是pc那么就将list加到jpc链上 等到dischargejpc来回填
  if (target == fs->pc)  /* 'target' is current position? */
    vsK_patchtohere(fs, list);  /* add list to pending jumps */
  // target不是pc立即回填
  else {
    // target一定小于pc 还没生成的指令不可能知道要跳转到还没生成的指令
    vs_assert(target < fs->pc);
...
void vsK_patchclose (FuncState *fs, int list, int level) {
  ...
  for (; list != NO_JUMP; list = getjump(fs, list)) {
    ...
    vs_assert(GET_OPCODE(fs->f->code[list]) == OP_JMP &&
...
static int vsK_code (FuncState *fs, Instruction i) {
  Proto *f = fs->f;
  dischargejpc(fs);  /* 'pc' will change */
  /* put new instruction in code array */
  // 确保空间足够
  vsM_growvector(fs->ls->L, f->code, fs->pc, f->sizecode, Instruction,
                  MAX_INT, "opcodes");
  // 将新的指令i放入code数组中
  f->code[fs->pc] = i;
  /* save corresponding line information */
  // 将行信息写入lineinfo数组中
  vsM_growvector(fs->ls->L, f->lineinfo, fs->pc, f->sizelineinfo, int,
                  MAX_INT, "opcodes");
...
*/
// 生成iABC类型的指令
int vsK_codeABC (FuncState *fs, OpCode o, int a, int b, int c) {
  // 检查参数与指令码的一致性
  vs_assert(getOpMode(o) == iABC);  // 确保是iABC指令
  // 保证不存在B参数不为0且B参数不被使用的情况
  vs_assert(getBMode(o) != OpArgN || b == 0);
  // 保证不存在C参数不为0且C参数不被使用的情况
  vs_assert(getCMode(o) != OpArgN || c == 0);
  // ABC都在合适的范围
  vs_assert(a <= MAXARG_A && b <= MAXARG_B && c <= MAXARG_C);
  // 实际生成指令的代码
  return vsK_code(fs, CREATE_ABC(o, a, b, c));
}
...
int vsK_codeABx (FuncState *fs, OpCode o, int a, unsigned int bc) {
  // 指令类型可以是iABx或者iAsBx
  vs_assert(getOpMode(o) == iABx || getOpMode(o) == iAsBx);
  // 参数C不被使用
  vs_assert(getCMode(o) == OpArgN);
  // 参数Å和参数B在正确的范围
  vs_assert(a <= MAXARG_A && bc <= MAXARG_Bx);
  // 构建指令
  return vsK_code(fs, CREATE_ABx(o, a, bc));
}
...
static int codeextraarg (FuncState *fs, int a) {
  vs_assert(a <= MAXARG_Ax);
  return vsK_code(fs, CREATE_Ax(OP_EXTRAARG, a));
}
...
int vsK_codek (FuncState *fs, int reg, int k) {
  if (k <= MAXARG_Bx)  // k能够在OP_LOADK指令中放下
    return vsK_codeABx(fs, OP_LOADK, reg, k);
  else {  // 使用OP_LOADKX指令
    int p = vsK_codeABx(fs, OP_LOADKX, reg, 0); // 生成OP_LOADKX指令
...
void vsK_reserveregs (FuncState *fs, int n) {
  vsK_checkstack(fs, n);  // 确保有n个空间
  fs->freereg += n;
}
...
// 返回值是v在常量表的位置
// 这个函数并不能保证vs中每个函数的常量表中没有重复的值,例如
...
static int addk (FuncState *fs, TValue *key, TValue *v) {
  ...
  TValue *idx = vsH_set(L, fs->ls->h, key);  /* index scanner table */
  int k, oldsize;
  ...
  setivalue(idx, k);
  // 扩张Proto对象的常量表
  vsM_growvector(L, f->k, k, f->sizek, TValue, MAXARG_Ax, "constants");
  // 初始化常量表增加的空间
...
int vsK_stringK (FuncState *fs, TString *s) {
  TValue o;
...
int vsK_intK (FuncState *fs, vs_Integer n) {
  TValue k, o;
...
static int vsK_numberK (FuncState *fs, vs_Number r) {
  TValue o;
  setfltvalue(&o, r);
...
** or 'nresults' is VS_MULTRET (as any expression can satisfy that).
*/
// 该函数用于设置表达式e代表的函数调用或者...变量的返回值个数
// e是函数调用或可变参数 其他情况nresults一定是VS_MULTRET
// nresults是指返回值的个数 如果是-1说明是返回多个值 0不返回 1或更大就是返回的个数
// 对于e是VCALL类型 回填OP_CALL指令的C参数
// 对于e是VVARARG类型 回填OP_VARARG指令的A和B参数
void vsK_setreturns (FuncState *fs, expdesc *e, int nresults) {
  if (e->k == VCALL) {  /* expression is an open function call? 表达式是一个函数调用 */
    ...
    vsK_reserveregs(fs, 1);
  }
  else vs_assert(nresults == VS_MULTRET);
}
...
void vsK_setoneret (FuncState *fs, expdesc *e) {
  // 是函数调用
  if (e->k == VCALL) {  /* expression is an open function call? */
    /* already returns 1 value */
    // 已经返回了一个值 参数C一定是2
    vs_assert(GETARG_C(getinstruction(fs, e)) == 2);
...
//   原info存放指令不变
// 其他类型不做操作
void vsK_dischargevars (FuncState *fs, expdesc *e) {
  switch (e->k) {
    ...
    case VUPVAL: {  /* move value to some (pending) register */
      ...
      e->u.info = vsK_codeABC(fs, OP_GETUPVAL, 0, e->u.info, 0);
      e->k = VRELOCABLE;
      break;
    }
    ...
    case VINDEXED: {
      ...
      if (e->u.ind.vt == VLOCAL) {  /* is 't' in a register? 表t是局部变量 */
        ...
        vs_assert(e->u.ind.vt == VUPVAL);
        op = OP_GETTABUP;  /* 't' is in an upvalue */
      }
      // 一定是重定向了 现在不知道查询表的结果要放到哪
      e->u.info = vsK_codeABC(fs, op, 0, e->u.ind.t, e->u.ind.idx);
      e->k = VRELOCABLE;
      break;
    }
...
static void discharge2reg (FuncState *fs, expdesc *e, int reg) {
  ...
  switch (e->k) {
    ...
    case VFALSE: case VTRUE: {
      vsK_codeABC(fs, OP_LOADBOOL, reg, e->k == VTRUE, 0);
      break;
    }
    // 常量如字符串在识别时已经写入了常量表,只需要生成一条OP_LOADK指令
    case VK: {
      vsK_codek(fs, reg, e->u.info);
      break;
    }
    // 浮点数和整数都是先加入常量表,再生成OP_LOADK指令
    case VKFLT: {
      vsK_codek(fs, reg, vsK_numberK(fs, e->u.nval));
      break;
    }
    case VKINT: {
      vsK_codek(fs, reg, vsK_intK(fs, e->u.ival));
      break;
    }
    // 需要重定向的指令
    case VRELOCABLE: {
    ...
    case VNONRELOC: {
      if (reg != e->u.info)                           // 已经设置存放的位置和传入的位置不同
        vsK_codeABC(fs, OP_MOVE, reg, e->u.info, 0); // 新增一条OP_MOVE指令移动到reg的位置
      break;
    }
...
static void discharge2anyreg (FuncState *fs, expdesc *e) {
  ...
  if (e->k != VNONRELOC) {  /* no fixed register yet? */
    // freereg后移1位
    vsK_reserveregs(fs, 1);  /* get a register */
    // freereg-1是当前空闲的一个位置
...
static int code_loadbool (FuncState *fs, int A, int b, int jump) {
  vsK_getlabel(fs);  /* those instructions may be jump targets ?? */
  return vsK_codeABC(fs, OP_LOADBOOL, A, b, jump);
}
...
static void exp2reg (FuncState *fs, expdesc *e, int reg) {
  ...
  if (e->k == VJMP)  /* expression itself is a test? */
    vsK_concat(fs, &e->t, e->u.info);  /* put this jump in 't' list */
  ...
  if (hasjumps(e)) {  // 判断是否有truelist或者falselist
    ...
    if (need_value(fs, e->t) || need_value(fs, e->f)) {
      ...
      int fj = (e->k == VJMP) ? NO_JUMP : vsK_jump(fs);  // ??
      ...
      vsK_patchtohere(fs, fj);  // 新生成的跳转指令加入到jpc上
    }
    final = vsK_getlabel(fs);
...
void vsK_exp2nextreg (FuncState *fs, expdesc *e) {
  vsK_dischargevars(fs, e);  // 处理VLOCAL,VUPVAL等类型
...
int vsK_exp2anyreg (FuncState *fs, expdesc *e) {
  vsK_dischargevars(fs, e);
  if (e->k == VNONRELOC) {  /* expression already has a register? */
    if (!hasjumps(e))  /* no jumps? */
      return e->u.info;  /* result is already in a register */
    // hasjumps且是存在寄存器的临时值,直接将表达式结果写入临时值
    // 也可以直接调用vsK_exp2nextreg效果一样,但是会增加freereg一增一减的操作
  ...
  // 正常情况直接存放到下一个寄存器中
  vsK_exp2nextreg(fs, e);  /* otherwise, use next available register */
  return e->u.info;
}
...
void vsK_exp2anyregup (FuncState *fs, expdesc *e) {
...
** a constant.
*/
// 没有跳转链等价于vsK_dischargevars
// 有跳转链调用vsK_exp2anyreg
void vsK_exp2val (FuncState *fs, expdesc *e) {
  if (hasjumps(e))
    vsK_exp2anyreg(fs, e);
  else
    vsK_dischargevars(fs, e);
...
int vsK_exp2RK (FuncState *fs, expdesc *e) {
  vsK_exp2val(fs, e);
  ...
  switch (e->k) {  /* move constants to 'k' */
    ...
    case VKINT: e->u.info = vsK_intK(fs, e->u.ival); goto vk;
    case VKFLT: e->u.info = vsK_numberK(fs, e->u.nval); goto vk;
    case VK:
     vk:
      e->k = VK;
...
*/
// 将表达式ex的结果存放到表达式var中
void vsK_storevar (FuncState *fs, expdesc *var, expdesc *ex) {
  switch (var->k) {
    ...
    case VUPVAL: {
      int e = vsK_exp2anyreg(fs, ex);
      vsK_codeABC(fs, OP_SETUPVAL, e, var->u.info, 0);
      break;
    }
    // 生成OP_SETTABLE或者OP_SETTABUP指令设置表项的值
    case VINDEXED: {
      OpCode op = (var->u.ind.vt == VLOCAL) ? OP_SETTABLE : OP_SETTABUP;
      // OP_SETTABLE,OP_SETTABLE的B参数是RK,所以这里调用vsK_exp2RK
      int e = vsK_exp2RK(fs, ex);
      vsK_codeABC(fs, op, var->u.ind.t, var->u.ind.idx, e);
      break;
    }
    default: vs_assert(0);  /* invalid var kind to store */
  }
  freeexp(fs, ex);
}
...
// 保存e和key,生成OP_SELF指令
void vsK_self (FuncState *fs, expdesc *e, expdesc *key) {
  int ereg;
  vsK_exp2anyreg(fs, e);
  ...
  vsK_reserveregs(fs, 2);  /* function and 'self' produced by op_self */
  vsK_codeABC(fs, OP_SELF, e->u.info, ereg, vsK_exp2RK(fs, key));
  freeexp(fs, key);
}
...
static void negatecondition (FuncState *fs, expdesc *e) {
  ...
  vs_assert(testTMode(GET_OPCODE(*pc)) && GET_OPCODE(*pc) != OP_TESTSET &&
...
// vsK_posfix中也修改了truelist或falselist但是是从e1合并到e2,不包含引入新指令
//
...
void vsK_goiftrue (FuncState *fs, expdesc *e) {
  ...
  vsK_concat(fs, &e->f, pc);  /* insert new jump in false list */
  // 将truelist加入到fs->jpc中
  // 表达式为真,所有truelist中的指令回填下一条生成指令的位置
  vsK_patchtohere(fs, e->t);  /* true list jumps to here (to go through) */
  e->t = NO_JUMP;
}
...
*/
void vsK_goiffalse (FuncState *fs, expdesc *e) {
  int pc;  /* pc of new jump */
  vsK_dischargevars(fs, e);
  switch (e->k) {
  ...
  vsK_concat(fs, &e->t, pc);  /* insert new jump in 't' list */
  // 使用下一条指令回填falselist,并清空falselist
  vsK_patchtohere(fs, e->f);  /* false list jumps to here (to go through) */
  e->f = NO_JUMP;
}
...
static void codenot (FuncState *fs, expdesc *e) {
  ...
  switch (e->k) {
    ...
    case VNONRELOC: {
      ...
      e->u.info = vsK_codeABC(fs, OP_NOT, 0, e->u.info, 0);
      e->k = VRELOCABLE;
      break;
    }
    default: vs_assert(0);  /* cannot happen */
  }
...
void vsK_indexed (FuncState *fs, expdesc *t, expdesc *k) {
  vs_assert(!hasjumps(t) && (vkisinreg(t->k) || t->k == VUPVAL));
  // 保存表t所在的位置
  t->u.ind.t = t->u.info;  /* register or upvalue index */
  // 保存k所在位置 可能在寄存器也可能在常量表 常量的话第8位被标记为1
  t->u.ind.idx = vsK_exp2RK(fs, k);  /* R/K index for key */
  // 标记该表是在upvalue还是局部变量
...
static int validop (int op, TValue *v1, TValue *v2) {
  switch (op) {
    ...
    case VS_OPSHL: case VS_OPSHR: case VS_OPBNOT: {  /* conversion errors */
      vs_Integer i;
      return (tointeger(v1, &i) && tointeger(v2, &i));
    }
    // 除法v2不能是0
    case VS_OPDIV: case VS_OPIDIV: case VS_OPMOD:  /* division by 0 */
      return (nvalue(v2) != 0);
    // 其他操作都是有效的
...
static int constfolding (FuncState *fs, int op, expdesc *e1,
  ...
  // 执行运算 结果写入res
  vsO_arith(fs->ls->L, op, &v1, &v2, &res);  /* does operation */
  if (ttisinteger(&res)) {  // 计算结果是整数
    e1->k = VKINT;
    e1->u.ival = ivalue(&res);
  }
  else {  /* folds neither NaN nor 0.0 (to avoid problems with -0.0) */
    vs_Number n = fltvalue(&res);
    // 避免nan和-0.0
    if (vsi_numisnan(n) || n == 0)
      return 0;
    e1->k = VKFLT;
    e1->u.nval = n;
  }
...
static void codeunexpval (FuncState *fs, OpCode op, expdesc *e, int line) {
  // 将表达式放入寄存器
  int r = vsK_exp2anyreg(fs, e);  /* opcodes operate only on registers */
  freeexp(fs, e);
  // 生成指令
  e->u.info = vsK_codeABC(fs, op, 0, r, 0);  /* generate opcode */
  e->k = VRELOCABLE;  /* all those operations are relocatable */
  vsK_fixline(fs, line);  // 添加生成指令所在行的信息
}
...
** Because 'vsK_exp2RK' can free registers, its calls must be
...
static void codebinexpval (FuncState *fs, OpCode op,
  ...
  // 先将两个操作数保存到RK中
  int rk2 = vsK_exp2RK(fs, e2);  /* both operands are "RK" */
  int rk1 = vsK_exp2RK(fs, e1);
  // 保存的操作数只是中间值,最终不需要
  freeexps(fs, e1, e2);
  e1->u.info = vsK_codeABC(fs, op, 0, rk1, rk2);  /* generate opcode */
...
** 'e1' was already put in R/K form by 'vsK_infix'.
*/
...
void vsK_prefix (FuncState *fs, UnOpr op, expdesc *e, int line) {
  static const expdesc ef = {VKINT, {0}, NO_JUMP, NO_JUMP};
  switch (op) {
    // 处理-和~ ef当作假的第二个操作数
    case OPR_MINUS: case OPR_BNOT:  /* use 'ef' as fake 2nd operand */
      // VS_OPUNM为12,OPR_MINUS为0
      // constfolding函数接收的是VS_OPUNM
      // OPR_MINUS和OPR_BNOT加上VS_OPUNM就能转换
      // 计算成功e保存结果
      if (constfolding(fs, op + VS_OPUNM, e, &ef))
        break;
      /* FALLTHROUGH */
...
void vsK_infix (FuncState *fs, BinOpr op, expdesc *v) {
  switch (op) {
    case OPR_AND: {
      // 对于and运算只有第一个操作符为true的时候才去执行第二个操作符
      // 所以这里是goiftrue
      vsK_goiftrue(fs, v);  /* go ahead only if 'v' is true */
      break;
    }
    case OPR_OR: {
      // 对于or运算只有第一个操作符为false才执行第二个操作符
      vsK_goiffalse(fs, v);  /* go ahead only if 'v' is false */
      break;
    }
    case OPR_CONCAT: {  // 连接操作将操作数一放入寄存器
      vsK_exp2nextreg(fs, v);  /* operand must be on the 'stack' */
      break;
    }
    ...
    case OPR_SHL: case OPR_SHR: {
      if (!tonumeral(v, NULL))
        vsK_exp2RK(fs, v);  // 不能转换成数字 将表达式v的值放入RK
      /* else keep numeral, which may be folded with 2nd operand */
      // 能转换成数字 这两个操作数可能会被合并
      break;
    }
    default: {
      vsK_exp2RK(fs, v);  // 其他操作符直接把值放入RK中
      break;
    }
  }
}
...
*/
// 为二元操作符生成最终的指令
void vsK_posfix (FuncState *fs, BinOpr op,
                  expdesc *e1, expdesc *e2, int line) {
  // 在vsK_infix已经处理过e1
  switch (op) {
    // vs的and和or运算符最终返回结果是两个操作数的其中之一,并不是true,false
    // and: 如果e1为假,最终结果是e1,否则最终结果是e2
    // or:  如果e1为真,最终结果是e1,否则最终结果是e2
    // 例如 1 and 10结果是10, 1 or 10结果是1
    case OPR_AND: {
      // 针对and操作,在vsK_infix已经将truelist加到fs->jpc上了
      vs_assert(e1->t == NO_JUMP);  /* list closed by 'vsK_infix' */
      vsK_dischargevars(fs, e2);
      // 将e1的falselist加入到e2的falselist后面
      vsK_concat(fs, &e2->f, e1->f);
      // e1拷贝为e2
      *e1 = *e2;
      break;
    }
    case OPR_OR: {
      // 针对or操作,在vsK_infix已经将truelist加到fs->jpc上了
      vs_assert(e1->f == NO_JUMP);  /* list closed by 'vsK_infix' */
      vsK_dischargevars(fs, e2);
      // 合并truelist 将e2拷贝进e1
      vsK_concat(fs, &e2->t, e1->t);
      *e1 = *e2;
      break;
    }
    // .. 连接操作可以进行优化
    ...
    case OPR_CONCAT: {  // e1一定在寄存器中
      ...
      if (e2->k == VRELOCABLE &&
        ...
        vs_assert(e1->u.info == GETARG_B(getinstruction(fs, e2))-1);
        freeexp(fs, e1);
        // 修改参数B
      ...
      // 正常情况的合并
        ...
        // 将e2保存到寄存器
        vsK_exp2nextreg(fs, e2);  /* operand must be on the 'stack' */
        // 生成OP_CONCAT指令
    ...
    case OPR_SHL: case OPR_SHR: {
      ...
      if (!constfolding(fs, op + VS_OPADD, e1, e2))
...
*/
void vsK_fixline (FuncState *fs, int line) {
...
** table (or VS_MULTRET to add up to stack top).
*/
// 生成OP_SETLIST指令
// 现在栈顶的情况是 T A A A,T是被设置的表,A是设置的值
// base是表的位置,nelems是表数组部分现有的个数加tostore,tostore是即将加入表中的个数
void vsK_setlist (FuncState *fs, int base, int nelems, int tostore) {
  int c =  (nelems - 1)/LFIELDS_PER_FLUSH + 1;  // 计算要设置到第几个块
  int b = (tostore == VS_MULTRET) ? 0 : tostore;  // 是VS_MULTRET说明不确定设置几个值
  // tostore要么是-1 要么是1-50不能是0
  vs_assert(tostore != 0 && tostore <= LFIELDS_PER_FLUSH);
  if (c <= MAXARG_C)
    vsK_codeABC(fs, OP_SETLIST, base, b, c);  // 生成指令
  else if (c <= MAXARG_Ax) {  // 参数C在一条指令存不开 使用OP_EXTRAARG指令存参数C
    vsK_codeABC(fs, OP_SETLIST, base, b, 0);  // C是0用来标记C存在下一个指令中
    codeextraarg(fs, c);
  }
  else  // 实在存不下 这表太大了
    vsX_syntaxerror(fs->ls, "constructor too long");
  // 释放掉原来存的要设置到表中的值
